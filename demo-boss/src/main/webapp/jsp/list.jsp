...
<body>
<div>
    <span class="fr"><a href="logout.htm">[登出]</a></span>
    <span class="fr">欢迎您：${admin.username}</span>
</div>
        ...
        <div class="search_box">
            <div class="search_sel">
                ...
                <select name="condition" id="condition">
                    ...
                    <option value="1" <c:if test="${selectBean.condition eq '1' }">selected="selected"</c:if>>商品名称
                    </option>
                    <option value="2" <c:if test="${selectBean.condition eq '2' }">selected="selected"</c:if>>商品编号
                    </option>
                </select>
...
<div id="goodsList">
    <form id="listForm" action="#" method="post">
        ...
        <table>
            ...
            <c:forEach var="goods" items="${pb.list}" varStatus="sta">
                <tr>
                    ...
                    <td><a href="javascript:void(0)"
                           onclick="location.href='toModifyGoods.htm?goodsId=${goods.goodsId}'">修改</a>|
                            <%-- <a href="javascript:void(0)" onclick="location.href='toModifyGoods.htm?goodsId=${goods.goodsId}'">修改</a>| --%>
                        <a href="javascript:void(0)"
                           onclick="location.href='delGoods.htm?goodsId=${goods.goodsId}'">删除</a>
            ...
            <tfoot>
            <tr>
                <td colspan="8">每页显示<input id="list_size" class="input1" type="text" value="${pb.pageSize}"
                                           onchange="changePageSize()">
                    条,共${pb.totalPages}页/当前${pb.pageNo}页,
                    跳转到<input type="text" class="input1" id="list_pageno" value="${pb.pageNo}"
                              onchange="changeShowPage(${pb.totalPages})">
                    ...
                    <c:forEach begin="${pb.startNo }" end="${pb.endNo}" varStatus="sta">
                        ...
                        <c:if test="${sta.count+pb.startNo-1==pb.pageNo}">
                            <a class="current" href="javascript:void(0)"
                               onclick="clickShowPage(${pb.pageSize},${pb.startNo+sta.count-1})">${pb.startNo+sta.count-1}</a>
                        </c:if>
                        <c:if test="${sta.count+pb.startNo-1!=pb.pageNo}">
                            <a href="javascript:void(0)"
                               onclick="clickShowPage(${pb.pageSize},${pb.startNo+sta.count-1})">${pb.startNo+sta.count-1}</a>
